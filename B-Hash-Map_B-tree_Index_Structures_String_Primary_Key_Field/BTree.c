...
)
{
    ...
    if ( record_position < CurrentNode->array_stored_elements )
    {
        ...
        compare_primary_key = strcmp( Get_Record_PrimaryKey(CurrentNode->RecordDataArray[record_position]),
                                      Get_Record_PrimaryKey(RecordData) );


        if (compare_primary_key == 0)
        {
...
)
{
    ...
    return BTree_Binary_Search(array_stored_elements, RecordDataArray, PrimaryKey, Get_Record_PrimaryKey);
...
)
{
    ...
    if (strcmp(PrimaryKey, Get_Record_PrimaryKey(RecordDataArray[array_stored_elements - 1])) > 0 )
    {
    ...
    if ( strcmp(PrimaryKey, Get_Record_PrimaryKey(RecordDataArray[0])) < 0 )
    {
    ...
    while (first_index <= last_index)
    {
        ...
        if ( strcmp(Get_Record_PrimaryKey(RecordDataArray[middle_index]), PrimaryKey) < 0 )
        {
        ...
        else if ( strcmp(Get_Record_PrimaryKey(RecordDataArray[middle_index]), PrimaryKey) == 0 )
        {
...
)
{
    ...
    for (int i = 0; i < CurrentNode->array_stored_elements; i++)
    {
        ...
        printf("Search Operation - Primary Key Field: (%s)\n", Get_Record_PrimaryKey(CurrentNode->RecordDataArray[i]));


        if ( strcmp(PrimaryKey, Get_Record_PrimaryKey(CurrentNode->RecordDataArray[i])) <= 0 )
        {
    ...
    if (!CurrentNode->leaf_type)
    {
        ...
        return BTreeSearch_Tool(CurrentNode->NodeLinkArray[record_position], PrimaryKey, Get_Record_PrimaryKey);
...
)
{
    ...
    if ( (record_position < CurrentNode->array_stored_elements)
         && ( strcmp(Get_Record_PrimaryKey(CurrentNode->RecordDataArray[record_position]), PrimaryKey) == 0 ) )
    {
    ...
    if (!CurrentNode->leaf_type)
    {
        ...
        return BTreeFastSearch_Tool(CurrentNode->NodeLinkArray[record_position], PrimaryKey, Get_Record_PrimaryKey);
...
)
{
    ...
    if (CurrentNode->leaf_type)
    {
        ...
        for (int i = 0; i < CurrentNode->array_stored_elements; i++)
        {
            ...
            printf(" (Leaf Node): (%s)", Get_Record_PrimaryKey(CurrentNode->RecordDataArray[i]));
...
)
{
    ...
    if (BTree->Root->leaf_type)
    {
        ...
        if ( (record_position < BTree->Root->array_stored_elements)
             && ( strcmp(Get_Record_PrimaryKey(BTree->Root->RecordDataArray[record_position]), PrimaryKey) == 0 ) )
        {
...
)
{
    ...
    if ( (deletion_position < NextLevelNode->array_stored_elements)
         && ( strcmp(Get_Record_PrimaryKey(NextLevelNode->RecordDataArray[deletion_position]), PrimaryKey) == 0 ) )
    {
...
)
{
    ...
    NewRecordData->RecordData = (TableB_RecordData)(malloc(sizeof(struct TableB_RecordData)));
    NewRecordData->table_id = 2;

    ((TableB_RecordData)(NewRecordData->RecordData))->Record_PrimaryKey_Size = Record_PrimaryKey_Size;
    ((TableB_RecordData)(NewRecordData->RecordData))->Record_PrimaryKey = (BTreePrimaryKey)(malloc(Record_PrimaryKey_Size * sizeof(char)));


    for (int i = 0; i < ((TableB_RecordData)(NewRecordData->RecordData))->Record_PrimaryKey_Size; i++)
    {

        ((TableB_RecordData)(NewRecordData->RecordData))->Record_PrimaryKey[i] = Record_PrimaryKey[i];

    }


    ((TableB_RecordData)(NewRecordData->RecordData))->Field_Data = Field_Data;
...
)
{
    ...
    return ((TableB_RecordData)(Input_RecordData->RecordData))->Record_PrimaryKey;
...
)
{
    ...
    return ((TableB_RecordData)(Input_RecordData->RecordData))->Field_Data;
...
)
{
    ...
    for (int i = 0; i < CurrentNode->array_stored_elements; i++)
    {
        ...
        BTreeSelectRecordData_ASC_Tool(CurrentNode->NodeLinkArray[i], SelectionResultList, Field_Data, Get_Record_FieldData);
...
)
{
    ...
    BTreeSelectRecordData_DESC_Tool(BTree.Root, SelectionResultList, Field_Data, Get_Record_FieldData);
...
)
{
    ...
    for (int i = 0; i < CurrentNode->array_stored_elements; i++)
    {
        ...
        BTreeSelectRecordData_DESC_Tool(CurrentNode->NodeLinkArray[i], SelectionResultList, Field_Data, Get_Record_FieldData);
...
)
{
    ...
    free(((TableB_RecordData)(RecordData_ToBe_Deleted->RecordData))->Record_PrimaryKey);
    free(((TableB_RecordData)(RecordData_ToBe_Deleted->RecordData)));
...
)
{
    ...
    *leaf_nodes_records = 0;
    ...
    BTreeComputeNodes_Tool(BTree.Root, internal_nodes, internal_nodes_records, leaf_nodes, leaf_nodes_records);
...
)
{
    ...
    *internal_nodes_records += CurrentNode->array_stored_elements;
    ...
    for (int i = 0; i < CurrentNode->array_stored_elements + 1; i++)
    {
        ...
        BTreeComputeNodes_Tool(CurrentNode->NodeLinkArray[i], internal_nodes, internal_nodes_records, leaf_nodes, leaf_nodes_records);
...
)
{
    ...
    for (int i = 0; i < CurrentNode->array_stored_elements + 1; i++)
    {
        ...
        BTreeCheckBalance_Tool(CurrentNode->NodeLinkArray[i], node_height + 1, BTree_height, unbalanced_nodes);
